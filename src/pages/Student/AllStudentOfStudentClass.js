...
const AllStudentOfStudentClass = () => {
  const [students, setStudents] = useState([]);
  const [toggle, setToggle] = useState(false);
  const ClassId = localStorage.getItem("student");
  ...
  const getAllStudents = async () => {
    try {
      const { data } = await axios.get(
        `${base_url}/api/v1/student/get-student/${ClassId}`
      );
      setStudents(data);
    } catch (error) {
...
export default AllStudentOfStudentClass;
